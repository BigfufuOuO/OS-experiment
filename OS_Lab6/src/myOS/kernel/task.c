...
void SchedulerFCFS(scheduler* sysSch);
void SchedulerPrio(scheduler* sysSch);
void SchedulerSJF(scheduler* sysSch);
...
//初始化就绪队列
void readyQueueInit_sys(myTCB* idleTask) {//对readyQueue进行初始化处理
    sysSch.schedulerInitFunc(idleTask);
}
...
void taskEnd(void) {
    //将一个在就绪队列中的TCB移除就绪队列
    sysSch.dequeueTaskFunc(currentTask);
...
int createTask(void (*taskBody)(void), taskPara paras) {
    ...
    sysSch.createTaskHook(task, taskBody);
...
void TaskManagerInit(void) {
    ...
    for (i = 0;i < TASK_NUM;i++) {//对进程池tcbPool中的进程进行初始化处理
        ...
        thisTCB->readyQueueNextTCB = (void*)0;
        thisTCB->waitingQueueNextTCB = (void*)0;
    }
    //创建idle任务
    idleTask = &tcbPool[0];
