...
int testdPkernel2(int argc, unsigned char **argv){
	unsigned long x,x1,x2,x3,xHandler;
	int i, tsize = 0x100;
	x = kmalloc(tsize); 
	if (x){
...
int testdPkernel1(int argc, unsigned char **argv){
	unsigned long x,x1,xHandler;
	int i, tsize = 0x100;
	x = kmalloc(tsize); 
	if (x){
...
int testeFPkernel(int argc, unsigned char **argv){
	...
	x = kmalloc(tsize); myPrintf(0x7,"X:0x%x:%d \n",x,tsize);
	if (x) {
		myPrintf(0x7, "We had successfully ");
		myPrintf(0x5, "malloc()");
		myPrintf(0x7, " a small memBlock (size=0x%x, addr=0x%x);\n", tsize,x);
...
void memTestCaseInit(void){	
	...
	addNewCmd("testdPkernel1\0", testdPkernel1, NULL, "Init a dPatition in kernel(size=0x100) [Alloc,Free]* with step = 0x20\0");
	addNewCmd("testdPkernel2\0", testdPkernel2, NULL, "Init a dPatition in kernel(size=0x100) A:B:C:- ==> -:B:C:- ==> -:C:- ==> - .\0");
	
	addNewCmd("testeFP\0", testeFP, NULL, "Init a eFPatition. Alloc all and Free all.\0");
	addNewCmd("testeFPkernel\0", testeFPkernel, NULL, "Init a eFPatition in kernel. Alloc all and Free all in kernel.\0");
}
